...
public class DanhSachDoiTuongSinhVien_3 {
    ...
    public static void main(String args[]) {
        ...
        List<SinhVien3> li = new ArrayList<>();
        for(int i=1;i<=n;i++){
            ...
            li.add(new SinhVien3(tmp, s[0], s[1], s[2], s[3]));
        }
...
class SinhVien3 implements Comparable<SinhVien3> {
    ...
    public SinhVien3(String MSV, String hoTen, String lop, String ngaySinh, String GPA){
        this.MSV=MSV;
        this.hoTen=hoTen;
        this.lop=lop;
    ...
    public String toString() {
        return String.format("%s %s %s %s %.2f", MSV, name(), lop, ns(), GPA);
    }
    @Override
    public int compareTo(SinhVien3 sv){
        if(this.GPA < sv.GPA){
            return -1;
        }else if(this.GPA > sv.GPA){
            return 1;
        }else{
            return 0;
        }
